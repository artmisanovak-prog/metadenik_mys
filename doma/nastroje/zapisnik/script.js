(function(){
  ...
  const CARDS_DATA = {
    ...
    "páv": { ilustrace: "doma/ilustrace/pav.jpg", text: "oko za okem. páv za pávem",
    ...
    "netopýr": { ilustrace: "doma/ilustrace/netopyr.jpg", text: "netopýří glamour",
    ...
    "motýl": { ilustrace: "doma/ilustrace/motyl.jpg", text: "motýl v břiše",
    ...
    "pavouk": { ilustrace: "doma/ilustrace/pavouk.jpg", text: "pavouk čeká",
    ...
    "krokodýl": { ilustrace: "doma/ilustrace/krokodyl.jpg", text: "krokodýlí slzy",
  ...
  function drawWheel(percentages) {
    ...
    const w=260, h=260, cx=130, cy=130, r=115;
  ...
  function renderCards() {
    ...
    currentCards.forEach(cardId => {
      ...
      // Kliknutí na kartu otevře odkaz
      ...
      uWrap.innerHTML = `<div class="uhlomer" id="uhlomer-${cardId}"></div>`;
      uhlomersRow.appendChild(uWrap);

      // Věta pod kartou (vytvoříme ji do samostatného prvku, který vložíme pod cardsBlock)
    });
    // Věty pod kartami
    const inspireRow = document.createElement('div');
    inspireRow.className = 'uhlomers-row';
    inspireRow.style.marginTop = '0.5rem';
    currentCards.forEach(cardId => {
      const inspireDiv = document.createElement('div');
      inspireDiv.className = 'inspire-sentence';
      inspireDiv.id = `inspire-${cardId}`;
      inspireDiv.style.flex = '1';
      inspireRow.appendChild(inspireDiv);
    });
    // Nahradíme starou inspireRow
    const oldInspire = document.querySelector('.inspire-row');
    if (oldInspire) oldInspire.remove();
    inspireRow.classList.add('inspire-row');
    cardsWrapper.appendChild(inspireRow);
  }
  ...
  function saveEntry() {
    const text = diaryEntry.value.trim();
    if (!text || !currentAnalysis) return alert('Analyzuj text.');
  ...
  function showHistory(entry) {
    ...
    historyDetail.innerHTML = `
      <div class="az-header"><span>${entry.title}</span><span>${new Date(entry.date).toLocaleString()}</span></div>
      <div style="padding:1rem">${entry.text.slice(0,300)}…</div>
      <div class="result-actions" style="margin-top:0">
        <button class="btn btn-g load-entry" data-id="${entry.id}">📂 Načíst</button>
        <button class="btn btn-g delete-entry" data-id="${entry.id}">🗑 Smazat</button>
      </div>`;
  }
  ...
  function init() {
    ...
    document.getElementById('newEntryBtn').addEventListener('click', () => {
      if (confirm('Smazat aktuální text?')) { diaryEntry.value=''; analysisResults.style.display='none'; currentAnalysis=null; currentEntryId=null; drawWheel(); }
    });
    // Sdílení sady
    ...
    if (params.has('shared')) {
      try {
        ...
        renderCards();
        if (shared.analysis) { currentAnalysis = shared.analysis; displayAnalysis(); }
      } catch (e) {}
    }
  }
