(function(){
  ...
  const CARDS_DATA = {
    ...
    "zajíc": { ilustrace: "doma/ilustrace/zajic.jpg", text:"zajíc co prošel ohněm",
      uhly: {"absurdní":"oheň který nefunguje","jazykový":"negace přísloví","ironický":"odolnost tam kde se čeká slabost","filozofický":"podstata nezávislá na prostředí","naivní":"oběť která nehoří","groteskní":"zajíc není palivo","kulturní":"stereotyp naruby","introspektivní":"žádný oheň nepálí stejně","surrealistický":"skok skrz nemožné"}, odkaz:"" },
    "ryba": { ilustrace: "doma/ilustrace/ryba.jpg", text:"rybí rap",
      ...
      odkaz: "https://artmisanovak-prog.github.io/chces-mys/doma/data/uhly/ryba_uhly.html" },
    ...
    "pavouk": { ilustrace: "doma/ilustrace/pavouk.jpg", text:"pavouk čeká", uhly: {"absurdní":"pauza která se odmítá stát pokračováním","jazykový":"věta bez slovesa","ironický":"struktura bez děje","filozofický":"stav mezi začátkem a koncem","naivní":"ono se to nějak vyřeší","groteskní":"architektura váhání","kulturní":"estetika nedokončení","introspektivní":"nechávám věci být otevřené","surrealistický":"čas zachycený ve vlákně"}, odkaz:"" },
    "krokodýl": { ilustrace: "doma/ilustrace/krokodyl.jpg", text:"krokodýlí slzy", uhly: {"absurdní":"totální výprodej falešných slz","jazykový":"reklama jako vyznání","ironický":"emoce zlevněná na nulu","filozofický":"hodnota citů v tržní logice","naivní":"proč by si děti nemohly koupit balíček slz na hraní","groteskní":"marketing empatie","kulturní":"cynismus zabalený do slevy","introspektivní":"falešný pláč výhodně","surrealistický":"hypermarket predátorů"}, odkaz:"" }
  };
  ...
  let uhlomersVisible = false;

  // DOM
  const cardsBlock = document.getElementById('cardsBlock');
  const uhlomersRow = document.getElementById('uhlomersRow');
  const inspireRow = document.getElementById('inspireRow');
  const cardsWrapper = document.getElementById('cardsWrapper');
  const toggleUhlomersBtn = document.getElementById('toggleUhlomersBtn');
  ...
  wheelCanvas.addEventListener('click', (e) => {
    ...
    if (currentAnalysis) {
      ...
      highlightedTextDiv.innerHTML = `<span style="background:${color};padding:2px 0">${currentAnalysis.fullText.replace(/\n/g,'<br>')}</span>`;
    }
  });

  // ---------- KARTY A ÚHLOMĚRY ----------
  function renderCards() {
    cardsBlock.innerHTML = ''; uhlomersRow.innerHTML = ''; inspireRow.innerHTML = '';
    currentCards.forEach(cardId => {
      ...
      uWrap.innerHTML = `<div class="uhlomer" id="uhlomer-${cardId}"></div>`; uhlomersRow.appendChild(uWrap);
      const inspireDiv = document.createElement('div'); inspireDiv.className = 'inspire-sentence'; inspireDiv.id = `inspire-${cardId}`;
      inspireRow.appendChild(inspireDiv);
    });
  }
  ...
  function buildUhlomer(cardId, randomize) {
    ...
    const inspireDiv = document.getElementById(`inspire-${cardId}`);
    if (uhlomerDiv.children.length) return;
    const card = CARDS_DATA[cardId], uhly = card.uhly;
    ...
    for (let i=0; i<12; i++) {
      ...
      const seg = document.createElement('div'); seg.className = `segment seg-${i}`;
      seg.style.backgroundColor = ANGLE_COLORS[i];
      if (!uhly[angle]?.trim()) seg.classList.add('inactive');
      else seg.addEventListener('click', (e) => {
        e.stopPropagation();
      ...
      if (i === selected && !seg.classList.contains('inactive')) {
        inspireDiv.textContent = `${angle}: ${uhly[angle]}`; seg.classList.add('active-segment');
      }
    }
  }

  function toggleUhlomers() {
    uhlomersVisible = !uhlomersVisible;
    cardsWrapper.classList.toggle('uhlomers-visible', uhlomersVisible);
    toggleUhlomersBtn.textContent = uhlomersVisible ? '🔮 Schovat úhloměry' : '🔮 Zobrazit úhloměry';
    if (uhlomersVisible) currentCards.forEach(cid => buildUhlomer(cid, true));
  }
  ...
  function saveEntry() {
    ...
    const entry = { id: currentEntryId || Date.now()+'-'+Math.random().toString(36), date: new Date().toISOString(), title: title || 'Bez názvu', text, cards: [...currentCards], dominantAngles: currentAnalysis.dominant, percentages: currentAnalysis.percentages };
    ...
    renderTimeline(); currentEntryId = null;
  }
  function loadEntries() { const s = localStorage.getItem('mys-diary'); if (s) diaryEntries = JSON.parse(s); }
  function renderTimeline() {
    timelineMap.innerHTML = ''; entriesList.innerHTML = '';
    const total = diaryEntries.length;
    diaryEntries.slice().reverse().forEach((entry, idx) => {
      const width = 20 + (entry.dominantAngles?.length || 1) * 8;
      const stripe = document.createElement('div'); stripe.className = 'map-stripe';
      stripe.style.width = width + 'px';
      entry.dominantAngles?.forEach(ang => { const s = document.createElement('div'); s.style.flex = '1'; s.style.backgroundColor = ANGLE_COLORS[ang]; stripe.appendChild(s); });
      stripe.addEventListener('click', () => showHistory(entry));
      timelineMap.appendChild(stripe);
  ...
  function showHistory(entry) {
    ...
    historyDetail.innerHTML = `<div class="az-header"><span>${entry.title}</span><span>${new Date(entry.date).toLocaleString()}</span></div><div style="padding:1rem">${entry.text}</div>
      <div class="result-actions"><button class="btn btn-g load-entry" data-id="${entry.id}">📂 Načíst</button><button class="btn btn-g delete-entry" data-id="${entry.id}">🗑 Smazat</button></div>`;
  }
  document.addEventListener('click', e => {
    if (e.target.classList.contains('load-entry')) { const id = e.target.dataset.id; const entry = diaryEntries.find(e=>e.id===id); if(entry){ diaryEntry.value=entry.text; currentCards=entry.cards; renderCards(); currentEntryId=entry.id; historyDetail.style.display='none'; } }
    if (e.target.classList.contains('delete-entry')) { const id = e.target.dataset.id; diaryEntries = diaryEntries.filter(e=>e.id!==id); localStorage.setItem('mys-diary', JSON.stringify(diaryEntries)); renderTimeline(); historyDetail.style.display='none'; }
  });

  // ---------- INIT ----------
  function init() {
    loadEntries(); renderCards(); renderTimeline();
    document.getElementById('shuffleCards').addEventListener('click', ()=>{ const ids=Object.keys(CARDS_DATA); currentCards=[ids[Math.floor(Math.random()*ids.length)], ids[Math.floor(Math.random()*ids.length)], ids[Math.floor(Math.random()*ids.length)]]; renderCards(); if(uhlomersVisible) currentCards.forEach(cid=>buildUhlomer(cid,true)); });
    document.getElementById('toggleUhlomersBtn').addEventListener('click', toggleUhlomers);
    document.getElementById('analyzeBtn').addEventListener('click', performAnalysis);
    document.getElementById('saveBtn').addEventListener('click', saveEntry);
    document.getElementById('newEntryBtn').addEventListener('click', ()=>{ if(confirm('Smazat aktuální text?')){ diaryEntry.value=''; analysisSection.style.display='none'; currentAnalysis=null; currentEntryId=null; drawWheel(); } });
    document.getElementById('lockShareBtn').addEventListener('click', ()=>{ const url=new URL(window.location); url.searchParams.set('cards', currentCards.join(',')); navigator.clipboard?.writeText(url.toString()); alert('Odkaz na sadu karet zkopírován.'); });
    document.getElementById('shareEntryBtn').addEventListener('click', ()=>{ if(!currentAnalysis) return; const data={text:diaryEntry.value, cards:currentCards, analysis:currentAnalysis}; const url=new URL(window.location); url.searchParams.set('shared', encodeURIComponent(JSON.stringify(data))); navigator.clipboard?.writeText(url.toString()); alert('Odkaz na zápis zkopírován.'); });
    document.getElementById('printEntryBtn').addEventListener('click', ()=>{ if(!currentAnalysis) return; const w=window.open('','','width=800,height=600'); w.document.write(`<pre>${diaryEntry.value}</pre><hr><pre>${JSON.stringify(currentAnalysis.percentages,null,2)}</pre>`); w.document.close(); w.print(); });
